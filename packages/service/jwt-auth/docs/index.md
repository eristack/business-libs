...
| An identity provider | No OAuth **authorization server**, no SAML, no email delivery, no MFA enrollment. For “Sign in with Google/OIDC” use [**`@eristack/oauth/client`**](/docs/oauth/getting-started) → then `issueTokens`; see [Tokens & refresh](./tokens-and-refresh.md#issue-without-a-password). |
