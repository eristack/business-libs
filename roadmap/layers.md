...
| 02 | **ISO** | Standard code lists — validate/normalize only; one npm package per standard | iso-3166, iso-4217, iso-639, unlocode (all **candidate** — see [Horizon](./horizon.md)) |
...
**Filesystem:** `packages/primitive` → `packages/iso` → `capability` → `service` → `infrastructure` → `ui` → `ai`. Layer 07 Features has no packages yet.

**Today:** apps compose layers 01–05 (and AI tooling). ISO layer and payment-manager are **planned** — brainstorm: [`_ai-docs/brainstorm/iso-layer-and-payment-manager.md`](../_ai-docs/brainstorm/iso-layer-and-payment-manager.md).

## Next candidates (backlog)

| Layer | Idea | When |
| --- | --- | --- |
| ISO | iso-3166, unlocode, reference-data seeds | Forwarding / ID masters need validated codes |
