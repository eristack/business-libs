...
export const composePackageMoment: Record<ComposePackageSlug, string> = {
  ...
  "data-grid": "One list contract for PO search, filters, paging",
  ...
  pbac: "Document state rules: can receive, can approve, …",
  ...
  multitab: "ERP workspace: PO, lines, receipt in headless tabs",
...
export const composeBackend: ComposeCodeFile = {
  filename: "server/procurement.ts",
  lines: [
    ...
    { text: "" },
    { text: "// --- list POs (data-grid + epoch) ---", packages: ["data-grid", "epoch"] },
    { text: "app.get('/api/purchase-orders', require('orders.read'), async (req, res) => {", packages: ["rbac", "data-grid"] },
    { text: "  const search = toSearch(grid.parse(req.query))", packages: ["data-grid"] },
    { text: "  const { policy } = await epoch.resolveCachePolicy('orders', req.query.clientEpoch)", packages: ["epoch"] },
    { text: "  const result = await executeDrizzleList({ db, schema, search, baseQuery: poQuery })", packages: ["data-grid"] },
    { text: "  res.json({ ...result, cachePolicy: policy })", packages: ["data-grid", "epoch"] },
    { text: "})", packages: ["data-grid"] },
    { text: "" },
    { text: "// --- create PO (doc-number, qups, money, pbac) ---", packages: ["doc-number", "qups", "money", "pbac"] },
    { text: "app.post('/api/purchase-orders', require('orders.create'), async (req, res) => {", packages: ["rbac"] },
    { text: "  await pbac.authorize('purchase-order.can-edit-draft', { document: draft })", packages: ["pbac"] },
    { text: "  const number = await docNumber.next('purchase-order')", packages: ["doc-number"] },
    ...
    { text: "  await db.insert(purchaseOrders).values({ number, total: total.toJSON() })", packages: ["money"] },
    { text: "  await epoch.bump('orders')", packages: ["epoch"] },
    { text: "  res.status(201).json({ number, lines, total })", packages: ["doc-number", "qups"] },
    { text: "})", packages: ["doc-number"] },
    { text: "" },
    { text: "// --- approve (rbac + pbac) ---", packages: ["rbac", "pbac"] },
    { text: "app.post('/api/purchase-orders/:id/approve', require('orders.approve'), async (req, res) => {", packages: ["rbac"] },
    { text: "  const po = await loadPo(req.params.id)" },
    { text: "  await pbac.authorize('purchase-order.can-approve', { document: po })", packages: ["pbac"] },
    { text: "  await db.update(purchaseOrders).set({ status: 'approved' })" },
    { text: "  await epoch.bump('orders')", packages: ["epoch"] },
    { text: "  res.json({ ok: true })" },
    { text: "})", packages: ["pbac"] },
    { text: "" },
    { text: "// --- goods receipt (stock, valuations, abac, hash chain) ---", packages: ["stock-movement", "valuations", "abac", "hash-chained-ledger"] },
    { text: "app.post('/api/goods-receipts', require('receipts.create'), async (req, res) => {", packages: ["rbac"] },
    { text: "  await pbac.authorize('purchase-order.can-receive', { document: po })", packages: ["pbac"] },
    { text: "  await abac.authorize('goods-receipt.book-value-limit', { subject, resource: receipt })", packages: ["abac"] },
    { text: "  await stock.append({ locationId, lotId, qty, entryTypeId: po.id })", packages: ["stock-movement", "hash-chained-ledger"] },
    { text: "  await valuations.receive({ key: { productId, currency }, qty, unitCost })", packages: ["valuations", "hash-chained-ledger"] },
    { text: "  await epoch.bump('orders'); await epoch.bump('inventory')", packages: ["epoch"] },
    ...
    { text: "async function postReceiptToGl(receipt: Receipt) {", packages: ["financial-ledger"] },
    { text: "  const inventory = Money.of(receipt.total, 'USD')", packages: ["money"] },
    { text: "  await ledger.post({", packages: ["financial-ledger", "hash-chained-ledger"] },
    { text: "    accountId: '1300-inventory', currency: 'USD',", packages: ["financial-ledger"] },
    { text: "    debit: inventory, credit: Money.of('0', 'USD'),", packages: ["money", "financial-ledger"] },
    { text: "    entryType: 'goods-receipt', entryTypeId: receipt.id," },
    { text: "  })", packages: ["financial-ledger"] },
    { text: "  await ledger.verify('1300-inventory:USD')", packages: ["financial-ledger", "hash-chained-ledger"] },
    { text: "}" },
  ],
};
...
export const composeFrontend: ComposeCodeFile = {
  filename: "src/routes/purchase-order.tsx",
  lines: [
    ...
    { text: "" },
    { text: "export function PurchaseOrderRoute() {" },
    ...
    { text: "  const policy = useEpochCachePolicy('orders')", packages: ["epoch"] },
    { text: "  const list = useDataGridList({", packages: ["data-grid"] },
    { text: "    queryKey: ['purchase-orders', search],", packages: ["data-grid"] },
    { text: "    staleTime: policy === 'use-cache' ? 60_000 : 0,", packages: ["epoch", "data-grid"] },
    { text: "    fetcher: (q) => poClient.list(q),", packages: ["data-grid"] },
    { text: "  })", packages: ["data-grid"] },
    { text: "" },
    ...
    { text: "" },
    { text: "  // --- approve (rbac + pbac) ---", packages: ["rbac", "pbac"] },
    { text: "  const canApprove = useCan(subject, 'orders.approve')", packages: ["rbac"] },
    { text: "  const approvePolicy = useBusinessPolicy('purchase-order.can-approve', po)", packages: ["pbac"] },
    { text: "" },
    { text: "  // --- receipt limit (abac) ---", packages: ["abac"] },
    { text: "  const bookLimit = usePolicy('goods-receipt.book-value-limit', { subject, resource })", packages: ["abac"] },
    { text: "" },
    ...
    { text: "    : poClient", packages: ["backseat"] },
    { text: "" },
    { text: "  return (", packages: ["multitab"] },
    { text: "    <MultitabWorkspace tabs={[", packages: ["multitab"] },
    { text: "      { id: 'po', title: po.number, panel: <PoList list={list} /> },", packages: ["multitab", "data-grid"] },
    { text: "      { id: 'edit', title: 'New PO', panel: <LineForm form={lineForm} total={total} /> },", packages: ["multitab", "qups", "money"] },
    { text: "      { id: 'gr', title: 'Receipt', panel: <ReceiptForm limit={bookLimit} /> },", packages: ["multitab", "abac"] },
...
export const composeTerminal: ComposeCodeFile = {
  filename: "terminal",
  lines: [
    { text: "# Load agent routing before you touch procurement.ts", packages: ["ai-knowledge"] },
    ...
    { text: "" },
    { text: "# Sprint memory on disk — PO receipt epic", packages: ["ai-workflow"] },
...
export const composeSteps: ComposeStep[] = [
  ...
  },
  {
    id: "approve",
    title: "Approve",
    moment: "Manager role plus document-state policy before status change.",
  ...
  },
  {
    id: "receive",
    title: "Goods receipt",
    moment: "Stock + FIFO on hash chain; ABAC book-value ceiling.",
    packages: [
      "stock-movement",
      "valuations",
      "hash-chained-ledger",
  ...
  },
  {
    ...
    moment: "Multitab ERP shell; Backseat swaps in-browser for the API.",
